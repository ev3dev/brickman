...
namespace Bluez5 {
    public class Adapter : Object {
        static HashTable<string, weak Adapter> adapter_map;

        static construct {
            adapter_map = new HashTable<string, weak Adapter> (str_hash, str_equal);
        }
        ...
        ~Adapter () {
            adapter_map.remove (((DBusProxy)dbus_proxy).g_object_path);
        }
